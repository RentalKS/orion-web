...
import { Reservation, ReservationDetails, ReservationResponse } from "../../types/api";
...
export async function updateReservation(reservationId: number, updatedData: Partial<Reservation>): Promise<Reservation> {
  return api.put<Reservation>(`/api/reservation/${reservationId}`, updatedData, {
    headers: {
      'Authorization': `Bearer ${STATIC_TEST_TOKEN}`,
      'Content-Type': 'application/json',
    },
...
export async function getReservationDetails(bookingId: number): Promise<ReservationDetails> {
  try {
    const response = await api.get<ReservationDetails>(`/api/reservation/details/${bookingId}`, {
      headers: {
        'Authorization': `Bearer ${STATIC_TEST_TOKEN}`,
        'Content-Type': 'application/json',
      },
