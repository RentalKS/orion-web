...
export const DashboardDailyPlan: React.FC<DashboardDailyPlanProps> = ({
  reservations,
}) => {
  ...
  const filteredReservations = date
    ? reservations.filter(
        (res) => dayjs(res.createdAt).isSame(date, "day") && res.onGoing > 0
      )
  ...
  return (
    <Card
      title="Daily Plan"
      extra={<DatePicker onChange={handleDateChange} />}
      style={{ maxWidth: "400px", width: "400px", marginRight: "2rem" }}
    >
      <List
