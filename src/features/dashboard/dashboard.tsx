...
export const Dashboard = () => {
    ...
    const fetchDashboardData = async (from?: number, to?: number) => {
        try {
            ...
            const [dashboardRes, vehiclesRes, paymentsRes] = await Promise.all([
                api.post<{ data: ReservationData[] }>("/api/dashboard", requestBody, { headers }),
                api.post<{ data: VehicleData[] }>("/api/dashboard/vehicles", requestBody, { headers }),
                api.post<{ data: PaymentData[] }>("/api/dashboard/payments", requestBody, { headers }),
            ]);
    ...
    if (loading) return <Spin size="large" style={{ display: "block", margin: "auto", marginTop: "5rem" }} />;
            ...
            <Flex justify="space-between">
                <Title level={5}>
                    {dateRange[0] && dateRange[1]
                        ? `Showing data from ${dayjs(dateRange[0]).format("YYYY-MM-DD")} to ${dayjs(dateRange[1]).format("YYYY-MM-DD")}`
                        : "Showing all data"}
                </Title>
            </Flex>
