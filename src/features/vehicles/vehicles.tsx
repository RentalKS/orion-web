import {
  ...
  import { useVehicles, useModels, useRateDates, useLocations } from "../../hooks/operations/useFetch";
  ...
  export const Vehicles = () => {
    ...
    const { data, isLoading, isError, error } = useVehicles({ page: 1, size: 10, search });
    ...
    const modelOptions = modelsData?.data?.data?.map((m) => ({ label: m.name, value: m.id })) || [];
    const rateDateOptions = rateDatesData?.data?.map((r) => ({ label: r.name, value: r.id })) || [];
    const locationOptions = locationsData?.data?.data?.map((l) => ({ label: l.address, value: l.id })) || [];
  
    // Placeholder vehicle image
    const defaultImage = "https://via.placeholder.com/300x200?text=Vehicle";
    
    // Card data mapping
    const cardData = data?.data?.data.map((vehicle) => ({
    ...
    return (
      <>
        ...
        <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
          ...
          <Col>
            <Button type="primary" icon={<Plus size={20} />} onClick={() => setIsDrawerOpen(true)}>
              Add Vehicle
            </Button>
          </Col>
        </Row>
      ...
      <Row gutter={[16, 16]}>
        {cardData.map((vehicle) => (
          <Col key={vehicle.id} xs={24} sm={12} md={8} lg={6}>
            <Card
              cover={
                <div
                  style={{
                    width: "100%",
                    height: "130px",
                    background: "linear-gradient(135deg,rgba(74, 108, 141, 0.47),rgba(169, 183, 204, 0.14))", // Gradient from dark blue to lighter blue
                    display: "flex",
                    alignItems: "center",
              ...
              actions={[
                <EditOutlined key="edit" onClick={() => handleEditVehicle(vehicle)} />,
                <DeleteOutlined key="delete" onClick={() => handleDeleteVehicle(vehicle.id)} />,
              ]}
              hoverable
            ...
            >
              ...
              <Descriptions column={1} bordered size="small">
                <Descriptions.Item label="Registration">{vehicle.registrationNumber}</Descriptions.Item>
                <Descriptions.Item label="Year">
                <span style={{ fontWeight:'lighter',fontSize:13 }}>
                  {vehicle.year}
                </span>
                </Descriptions.Item>
                <Descriptions.Item label="Contract Number">{vehicle?.contractVehicleNumber}</Descriptions.Item>
                <Descriptions.Item label="Fuel">
                  <Tag color={vehicle.fuelType === "DIESEL" ? "volcano" : "green"}>{vehicle.fuelType}</Tag>
                </Descriptions.Item>
                <Descriptions.Item label="Mileage">
                  <span style={{ fontWeight:'lighter',fontSize:13 }}>{vehicle.mileage.toLocaleString()} km</span>

                </Descriptions.Item>
                <Descriptions.Item label="Transmission">
                <span style={{ fontWeight:'lighter',color: "grey",fontSize:12 }}>
                ...
                <Descriptions.Item label="Status">
                  <Tag color={vehicle.vehicleStatus === "RESERVED" ? "orange" : "green"}>
        ...
        <Drawer
          ...
          footer={
            <Button type="primary" onClick={selectedVehicle ? handleUpdateVehicle : handleCreateVehicle}>
        ...
        >
          <Form form={form} layout="vertical">
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item label="Rate Date" name="rateId" rules={[{ required: true }]}>
                  <Select options={rateDateOptions} placeholder="Select Rate Date" />
                </Form.Item>
              </Col>
              <Col span={12}>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item label="Policy Number" name={["insurancePolicy", "policyNumber"]} rules={[{ required: true }]}>
                  <Input placeholder="Enter policy number" />
                </Form.Item>
              </Col>
              <Col span={12}>
                <Form.Item label="Provider Name" name={["insurancePolicy", "providerName"]} rules={[{ required: true }]}>
                  <Input placeholder="Enter insurance provider" />
                </Form.Item>
              </Col>
            </Row>
            <Form.Item label="Coverage Details" name={["insurancePolicy", "coverageDetails"]}>
